...
const api_key_secret = process.env.API_KEY_SECRET || "exhibitit_default_secret";
...
app.post([apiBasePath + "/meeting"], (req, res) => {
  ...
  if (authorization != api_key_secret) {
    log.debug("exhibitit get meeting - Unauthorized", {
      header: req.headers,
      body: req.body,
    });
  ...
  log.debug("exhibitit get meeting - Authorized", {
    header: req.headers,
    body: req.body,
    meeting: meetingURL,
  });
