...
let recStartTime;
let recElapsedTime;
let exhibiitTheme = "neon"; // neon - dark - forest - ghost ...
...
function getHtmlElementsById() {
  ...
  selectheme = getId("exhibiitTheme");
...
function welcomeUser() {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      let message = {
        email: "",
        subject: "Please join our exhibitit conference",
      ...
      switch (event.channel.label) {
        case "exhibitit_chat_channel":
          try {
            ...
            console.error("exhibitit_chat_channel", err);
          }
          break;
        case "exhibitit_file_sharing_channel":
          try {
            let dataFile = msg.data;
            handleDataChannelFileSharing(dataFile);
          } catch (err) {
            console.error("exhibitit_file_sharing_channel", err);
          }
          break;
      }
    };
...
/**
 * Set exhibitit theme neon | dark | forest | sky | ghost | ...
 * @param {*} theme
 */
...
function setButtonsBarPosition(position) {
  ...
  switch (BtnsBar) {
    case "vertical":
      let btnsLeft = exhibiitTheme === "ghost" ? "5px" : "20px";
...
function handleAudio(e, init, force = null) {
  ...
  if (init) {
    ...
    if (!isMobileDevice) {
      setTippy(
        initAudioBtn,
        myAudioStatus ? "Click to audio OFF" : "Click to audio ON",
        "top"
      );
    }
  }
...
function handleVideo(e, init, force = null) {
  ...
  if (init) {
    ...
    if (!isMobileDevice) {
      setTippy(
        initVideoBtn,
        myVideoStatus ? "Click to video OFF" : "Click to video ON",
        "top"
      );
    }
  }
...
function setMyAudioStatus(status) {
  ...
  if (!isMobileDevice) {
    setTippy(
      audioBtn,
      status ? "Click to audio OFF" : "Click to audio ON",
      "right-start"
    );
  }
}
...
function setMyVideoStatus(status) {
  ...
  if (!isMobileDevice) {
    setTippy(
      videoBtn,
      status ? "Click to video OFF" : "Click to video ON",
      "right-start"
    );
  }
}
...
function handleRoomLocked() {
  ...
  Swal.fire({
    ...
    text: "The room is locked, try again in few minutes.",
...
function createFileSharingDataChannel(peer_id) {
  fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel(
    "exhibitit_file_sharing_channel"
  );
...
function handleDataChannelFileSharing(data) {
  ...
  receiveFilePercentage.innerHTML =
    "Receive progress: " +
    ((receivedSize / incomingFileInfo.fileSize) * 100).toFixed(2) +
    "%";
...
/**
 * Hide incoming file transfersvg
 */
...
function handleKickedOut(config) {
  ...
  Swal.fire({
    ...
    background: swalBackground,
    position: "center",
    title: "You will be removed from the room by",
    html:
      `<h2 style="color: red;">` +
      `User ` +
      peer_name +
      `</h2> after <b style="color: red;"></b> milliseconds.`,
    timer: 10000,
